...
class ResCompany(models.Model):
    ...
    def _l10n_ar_rg5329_validate_required_configuration(self):
        self.ensure_one()
        if not self.l10n_ar_rg5329_enabled:
            raise ValidationError(_("Debe habilitar el regimen RG 5329."))
        if not self.l10n_ar_rg5329_account_id:
            raise ValidationError(_("Debe configurar la cuenta contable de percepcion RG 5329."))
        if not self.l10n_ar_rg5329_tax_group_id:
    ...
    def action_l10n_ar_rg5329_sync_products(self):
        for company in self:
            company._l10n_ar_rg5329_validate_required_configuration()
    ...
    def _l10n_ar_rg5329_tax_has_entries(self, tax):
        self.ensure_one()
        if not tax:
            return False
        return bool(
    ...
    def _l10n_ar_rg5329_ensure_taxes(self):
        for company in self:
            ...
            for rate_key in RG5329_TAX_SPECS:
                ...
                if company._l10n_ar_rg5329_tax_has_entries(tax):
                    # Odoo does not allow changing tax computation fields once used.
                    # Keep the tax and use it as-is.
                    continue
    ...
    def _l10n_ar_rg5329_sync_products(self):
        """Clean RG 5329 perception taxes from products.

        The perception is computed dynamically on invoices. This method is used
        after enabling the regime or through the configuration button to remove
        old/manual perception taxes from product templates.
        """
        ...
        for company in self:
            ...
            templates._l10n_ar_rg5329_clean_perception_taxes(company)
