...
class AccountMove(models.Model):
    ...
    def _l10n_ar_rg5329_invoice_base_lines(self):
        """Return commercial/base invoice lines, compatible with Odoo 19.

        Odoo 19 may use display_type='product' for commercial invoice lines.
        We inspect both invoice_line_ids and line_ids because, during onchange
        and web_save, one recordset may be more up to date than the other.
        """
        self.ensure_one()
        ...
        _logger.debug(
            "RG5329 base lines detected: invoice_line_ids=%s line_ids=%s base=%s",
    ...
    def _l10n_ar_rg5329_collect_by_rate(self, lines=None):
        ...
        _logger.info(
            "RG5329 bases acumuladas por alicuota: %s",
    ...
    def _l10n_ar_rg5329_applicable_rate_keys(self, grouped_lines):
        ...
        for rate_key, data in grouped_lines.items():
            ...
            _logger.info(
                "RG5329 check rate=%s base_acumulada=%s percepcion_moneda_compania=%s minimo=%s",
                rate_key,
                base,
        ...
        _logger.info("RG5329 alicuotas aplicables: %s", applicable)
        return applicable
    ...
    def _l10n_ar_rg5329_sync_invoice_taxes(self):
        for move in self:
            ...
            if not lines:
                _logger.debug("RG5329: factura sin lineas base para evaluar")
                continue

            if not move._l10n_ar_rg5329_can_apply():
                _logger.info("RG5329: condiciones generales no alcanzadas, limpio percepciones")
                for line in lines:
