...
class ResCompany(models.Model):
    ...
    @api.model
    def _get_environment_type_padron(self):
        # Le cambiamos el nombre porque era el mismo que el modulo de FE nativo de Odoo
        # Pasó de _get_environment_type a _get_environment_type_padron
        """
    ...
    # En este caso el nombre con el módulo nativo de FE es casi el mismo por eso
    # zafamos de tener que cambiarlo "_get_key_and_certificate" (nombre nativo)
    ...
    def get_connection(self, afip_ws):
        ...
        environment_type = self._get_environment_type_padron()
    ...
    def _create_connection(self, afip_ws, environment_type):
        ...
        # because pyafipws wsaa looks for "BEGIN RSA PRIVATE KEY" we change key
    ...
    def authenticate(
        ...
        try:
            ...
            raise UserError(
                _(
                    "La libreria Python pyafipws no esta instalada en el servidor. "
                    "En Odoo.sh debe agregarse 'pyafipws' al requirements.txt del "
                    "repositorio y reconstruir el build."
                )
            )
