...
class AccountMove(models.Model):
    ...
    def _l10n_ar_rg5329_collect_by_rate(self, lines=None):
        ...
        _logger.info(
            "RG5329 V5 BASES ACUMULADAS POR ALICUOTA: %s",
    ...
    def _l10n_ar_rg5329_applicable_rate_keys(self, grouped_lines):
        ...
        for rate_key, data in grouped_lines.items():
            ...
            _logger.info(
                "RG5329 V5 CHECK rate=%s base_acumulada=%s percepcion_moneda_compania=%s minimo=%s",
                rate_key,
                base,
        ...
        _logger.info("RG5329 V5 ALICUOTAS APLICABLES: %s", applicable)
        return applicable
    ...
    def _l10n_ar_rg5329_sync_invoice_taxes(self):
        for move in self:
            ...
            if not lines:
                _logger.info("RG5329 V5: factura sin lineas base para evaluar")
                continue

            if not move._l10n_ar_rg5329_can_apply():
                _logger.info("RG5329 V5: condiciones generales no alcanzadas, limpio percepciones")
                for line in lines:
